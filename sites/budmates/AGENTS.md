...
- Editorial pages stay static: `/blogs/news`, `/pages/*`, `/policies/*` and `/kunnskap`. Brand chrome, cart, thank-you and 404 stay Worker-owned templates or static shells.
- Preserve Shopify public paths: `/cart`, `/search`, `/products/{handle}`, `/collections/{handle}`, `/pages/{handle}`, `/policies/{handle}` and `/blogs/news/{handle}`, without trailing slashes. Redirect former Worker paths using `routes.mjs`.
