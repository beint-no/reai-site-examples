...
function currencySwitcher(canonicalPath, context) {
  return `<nav class="currency-switch" aria-label="${escapeHtml(message(context, "currency_label"))}">${MARKETS.map((item) => {
    const current = item.handle === (context?.market || defaultMarketForLocale(context?.locale));
    const query = item.handle === defaultMarketForLocale(context?.locale) ? "" : `?market=${item.handle}`;
    return `<a href="${escapeHtml(`${publicPath(context, canonicalPath)}${query}`)}" data-market="${escapeHtml(item.handle)}"${current ? ' aria-current="true"' : ""}>${escapeHtml(item.currency)}</a>`;
  }).join("")}</nav>`;
}
