...
function chrome(store, active, canonicalPath, context) {
  ...
  const home = publicPath(context, "/");
  const core = publicPath(context, "/products/duofiller-core-g3/");
  const connections = publicPath(context, "/collections/addons/");
  ...
  const alternatePath = isNorwegian(context) ? canonicalPath : `/nb${canonicalPath}`;
...
export function renderCollectionPage(store, handle, context) {
  ...
  const body = `<header class="page-hero page-hero--compact"><div class="shell">${breadcrumbs([{ href: publicPath(context, "/"), label: message(context, "nav_home") }, { label: title }])}<p class="eyebrow">${escapeHtml(count)}</p><h1>${escapeHtml(title)}</h1><p>${escapeHtml(stripHtml(description))}</p></div></header><section class="section section-white catalog-section"><div class="catalog-layout shell"><nav class="catalog-nav" aria-label="${escapeHtml(message(context, "footer_shop"))}"><h2>${escapeHtml(message(context, "footer_shop"))}</h2>${navCollections(store).map((item) => `<a href="${publicPath(context, `/collections/${item.handle}/`)}"${item.handle === handle ? ' aria-current="page"' : ""}>${escapeHtml(item.title)}</a>`).join("")}<a href="${publicPath(context, "/collections/all/")}"${handle === "all" ? ' aria-current="page"' : ""}>${escapeHtml(message(context, "commerce_all_products"))}</a></nav><div><div class="catalog-toolbar"><span>${escapeHtml(count)}</span><a href="${publicPath(context, staticPaths(context).search)}">${escapeHtml(message(context, "nav_search"))} →</a></div><div class="product-grid" data-server-rendered="true">${products.map((product) => productCard(product, store, context)).join("")}</div></div></div></section>`;
...
function renderMessagePage(store, context, { title, heading, text, status }) {
  const body = `<section class="page-hero"><div class="shell"><p class="eyebrow">${escapeHtml(status)}</p><h1>${escapeHtml(heading)}</h1><p>${escapeHtml(text)}</p><div class="hero-actions"><a class="button button-primary" href="${publicPath(context, "/")}">${escapeHtml(message(context, "nav_home"))}</a><a class="button button-secondary" href="${publicPath(context, staticPaths(context).contact)}">${escapeHtml(message(context, "nav_contact"))}</a></div></div></section>`;
  return documentHtml({ title: `${title} | DuoFiller`, description: text, canonicalPath: "/404.html", body, store, context, robots: "noindex" });
}

export const renderNotFoundPage = (store, context) => renderMessagePage(store, context, { title: "404", heading: message(context, "commerce_not_found_title"), text: message(context, "commerce_not_found_text"), status: "404" });
export const renderUnavailablePage = (store, context) => renderMessagePage(store, context, { title: "503", heading: message(context, "commerce_unavailable_title"), text: message(context, "commerce_unavailable_text"), status: "503" });
