...
export const MARKETS = Object.freeze([
  { handle: "norway", currency: "NOK", symbol: "kr", label: "currency_market_norway" },
  { handle: "europe", currency: "EUR", symbol: "€", label: "currency_market_europe" },
  { handle: "international", currency: "USD", symbol: "$", label: "currency_market_international" },
]);
...
function currencySwitcher(context, canonicalPath) {
  const current = context?.market || defaultMarketForLocale(context?.locale);
  const selected = MARKETS.find((item) => item.handle === current) || MARKETS.at(-1);
  const options = MARKETS.map((item) => {
    const currentAttribute = item.handle === current ? ' aria-current="true"' : "";
    const marketHref = `${publicPath(context, canonicalPath)}${marketQueryFor(context?.locale, item.handle)}`;
    return `<li><a href="${escapeHtml(marketHref)}" data-market="${escapeHtml(item.handle)}"${currentAttribute}><span class="currency-option-symbol" aria-hidden="true">${escapeHtml(item.symbol)}</span><span class="currency-option-copy"><strong>${escapeHtml(item.currency)}</strong><small>${escapeHtml(message(context, item.label))}</small></span><svg class="currency-check" viewBox="0 0 24 24" aria-hidden="true"><path d="m5 12 4 4L19 6"></path></svg></a></li>`;
  }).join("");
  const label = message(context, "currency_label");
  return `<details class="currency-switch" data-market-switch data-label="${escapeHtml(label)}"><summary aria-label="${escapeHtml(label)}: ${escapeHtml(selected.currency)}"><span class="currency-indicator" data-market-current-symbol aria-hidden="true">${escapeHtml(selected.symbol)}</span><span class="currency-code" data-market-current-code>${escapeHtml(selected.currency)}</span><svg class="currency-chevron" viewBox="0 0 12 8" aria-hidden="true"><path d="m1.5 1.5 4.5 4.5 4.5-4.5"></path></svg></summary><div class="currency-popover"><p>${escapeHtml(label)}</p><ul>${options}</ul></div></details>`;
}
