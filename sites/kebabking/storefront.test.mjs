...
test("matches dynamic commerce routes", () => {
  ...
  assert.equal(storefront.matchRoute("/om-oss/"), null);
});
...
test("preserves the production homepage identity around the live catalog", () => {
  const home = storefront.renderHomePage(store);

  assert.match(home, /Kongen av/);
  assert.match(home, /Trondheim-kebab/);
  assert.match(home, /Brattørgata 4/);
  assert.match(home, /\/assets\/hero\.jpg/);
  assert.match(home, /\/assets\/mezze\.jpg/);
  assert.match(home, /data-en=/);
  assert.match(home, /href="\/handlekurv\/"/);
  assert.match(home, /Kebabtallerken/);
  assert.match(home, /199 kr/);
});
